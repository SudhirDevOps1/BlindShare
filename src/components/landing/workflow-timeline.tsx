...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    title: "AES-GCM-256 (extractable: false)",
    detail: "WebCrypto API imports the 256-bit key with extractable=false. Extension scrapers cannot extract it from memory. Raw bytes are wiped immediately (rawBytes.fill(0)).",
    code: "const key = await crypto.subtle.importKey(\n  'raw', rawBytes, 'AES-GCM', false, ['encrypt']\n);\nrawBytes.fill(0); // RAM wipe immediately!",
    codeColor: "#c4b5fd",
    tag: "Zero-Exfiltration",
    who: "owner",
  },
  {
    num: 3,
  ...
  },
  {
    ...
    detail: "The viewer's browser reads #k=, wipes raw bytes, derives slide keys via HKDF (RFC 5869), and decrypts locally with non-extractable keys + forensic steganography.",
    code: "const key = await importKey(rawKey, false);\nrawBytes.fill(0); // Wipe RAM\nconst slideKey = await deriveSlideKey(key, page);\n// → Decrypted & watermarked with forensic stego",
    codeColor: "#f0abfc",
