...
| **3** | **Version Synchronization** | All files (`package.json`, `version/route.ts`, `README.md`, `CHANGELOG.md`) must report `v1.4.1`. |
| **4** | **Automated Test Invariant** | Always run `npm run typecheck && npm test && npm run lint`. All 48 security tests must pass with 0 errors. |
