...
| Database dump theft / SQL injection read leak | AES-256-GCM Database Field Vault (`src/lib/crypto/db-vault.ts`) encrypts all PII (user/viewer emails, TOTP secrets, NDA signatures, slide Q&A, document titles, filenames, storage keys, link names, NDAs, watermarks, dataroom metadata) | Attacker observes row counts, timestamps, and relational links, but cannot read plaintext PII |
| Link password salt & wrapped doc-key metadata | Client-side PBKDF2 unwrap: `passwordSaltHex` and `wrappedKeyHex` are public by design so legitimate recipients can derive the unwrapping key in browser without server knowledge | Offline password cracking if viewer password has low entropy (mitigated by rate-limiting and recommended 16+ char passwords) |
